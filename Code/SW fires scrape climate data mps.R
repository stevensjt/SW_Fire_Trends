...
df <- read.csv(file="./R/Climate_Analysis/sdc_gee_1984_2017.csv", header=TRUE, sep=",")
df <- df[,-1] #remove first column (rownum)
#df <- read.csv(file="./R/Climate_Analysis/sdc_df_climate.csv", header=TRUE, sep=",")

wd <- c("D:/Patch_Metrics/GEE_Severity_Dataset")

for (r in c(401:593)){ 
  yr <- as.numeric(mid(
    str_extract(df[r,1][[1]],"[^_]+"), 14, 4
    ))
  ...
  wd_yr <- paste(wd,yr,"High_Severity","Forested_High_Severity",sep = "/")
  fire.name <- df[r,"fire.name"]
  hs.raster.raw <- raster(paste(wd_yr,fire.name,sep = "/"))
  hs.poly.raw <- raster::rasterToPolygons(hs.raster.raw, dissolve = TRUE)
  ...
  vs=c("tmmx","tmmn","rmax","bi", "erc") 
  vs_full=c("air_temperature","air_temperature","relative_humidity","burning_index_g", "energy_release_component-g") 
  ...
  for(index in 1:length(vs)){ #Variable for loop
    ...
    dest <- paste0("D:/Patch_Metrics/GEE_Severity_Dataset/R/Climate_Analysis/Data/",v,".nc")
    #fire.list[f,"lat"]=gCentroid(hs_fire_ll)$y
    #fire.list[f,"long"]=gCentroid(hs_fire_ll)$x
    tmp=try(download.file(url= v_link, destfile = dest, method = "curl", quiet=FALSE))
...
write.csv(df, "./R/Climate_Analysis/sdc_gee_1984_2017_climate_401_593.csv")
